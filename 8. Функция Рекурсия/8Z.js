...
2. Пусть шариков два и мы бросили один шарик с этажа номер k. Как мы будем действовать в зависимости от того, разобъется ли шарик или нет?
    Решение возможно для 1-4 этажных зданий.
    Если шарик разбился, следующий шар разбиваем на этажах от 1 до k. 
    Если шарик не разбился, следующий шар разбиваем на этажах от k до n. 
...
// метод половинного деения (метод бинарной бисекции)
function fall(n, k = 1) {
    if (n <= 1) {
        return 0;
    } else {
        return fall(Math.ceil(n / 2), Math.ceil(Math.ceil(n / 2) / 2)) + 1; // подсказки
    }
}
