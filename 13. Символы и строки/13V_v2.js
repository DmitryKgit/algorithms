...
function isIP(S) {
    ...
    if (S.length == 4) {                // длина адреса
        for (let item of S) {
            // проверка на длину и величину числа, а также на пустую строку, а также проверка на ведущий ноль
            if (Number(item) > 255 || item.length > 3 || item.length == 0 || item == '0' && item.length != 1) { 
                return false;
            } 
        }
        return true;
...
console.log(isIP(sym('.0.1.255')));       // NO
console.log(isIP(sym('-255.0.1.255')));   // NO
console.log(isIP(sym('1. 0.1.255')));     // NO
console.log(isIP(sym('255.0.11 .255')));  // NO
console.log(isIP(sym('255.0.11A.255 ')));
console.log(isIP(sym('255.0.11.0255'))); // NO
// Ввод и вывод данных
// function interface() { 
//     let S = '255.0.1.255';
//     console.log(isIP(S));
