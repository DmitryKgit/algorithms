...
function isIP(S) {
    S = S.split('.');
    if (S.length == 4) {                // длина адреса
        for (let item of S) {
            if (Number(item) > 255 || item.length > 3 || item.length == 0) { // проверка на длину и величину числа, а также на пустую строку
                return false;
            } else if (item == '0' && item.length != 1) { // проверка на ведущий ноль
                return false;
            }
        }
        return true;
...
function sym(str) {
    for (let item of str) {
        if (!isNumberOrPoint(item))
            return '';
    }
    return str;
}

// предикат для определения цифры и точек
function isNumberOrPoint(n) {
    return new Set("1234567890.").has(n);
}

console.log(isIP(sym('255.0.1.255')));
console.log(isIP(sym('.0.1.255')));       
console.log(isIP(sym('-255.0.1.255')));   
console.log(isIP(sym('1. 0.1.255')));     
console.log(isIP(sym('255.0.11 .255')));  
console.log(isIP(sym('255.0.11A.255 ')));
console.log(isIP(sym('255.0.11.0255'))); 
// Ввод и вывод данных
// function interface() { 
//     let S = '255.0.1.255';
//     console.log(isIP(S));
