...
function processing(input) {
    ...
    let max = 0;
    while (next != 0) {
        if (next > pre) {   // последовательность растёт
            inc++;
        } else {            // последовательность закончилась
            inc = 1;
        }
        if (next < pre) {   // последовательность убывает
            dec++;
        } else {            // последовательность закончилась
            dec = 1;
        }
        if (max < inc) {    // нахождение максимума
            max = inc;
        }
        if (max < dec) {    // нахождение максимума
            max = dec;
        }
        console.log('pre: ', pre, 'next: ', next, 'inc: ', inc, 'dec: ', dec,'max: ', max);
